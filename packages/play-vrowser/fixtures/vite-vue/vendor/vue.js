...
function genCacheKey(source, options) {
  return (
    source + JSON.stringify(options, (_, val) => (typeof val === 'function' ? val.toString() : val))
  )
}
...
const isGloballyAllowed = /* @__PURE__ */ makeMap(
  ...
  notify(debugInfo) {
    ...
    try {
      ...
      for (let link = this.subs; link; link = link.prevSub)
        if (link.sub.notify()) link.sub.dep.notify()
    } finally {
      endBatch()
    }
  }
...
var ObjectRefImpl = class {
  constructor(_object, _key, _defaultValue) {
    ...
    if (!isArray(_object) || !isIntegerKey(String(_key)))
      do shallow = !isProxy(obj) || isShallow(obj)
...
function setDevtoolsHook$1(hook, target) {
  ...
  if (devtools$1) {
    ...
    ;(target.__VUE_DEVTOOLS_HOOK_REPLAY__ = target.__VUE_DEVTOOLS_HOOK_REPLAY__ || []).push(
      newHook => {
...
const devtoolsComponentRemoved = component => {
  if (
    devtools$1 &&
    typeof devtools$1.cleanupBuffer === 'function' &&
...
function moveTeleport(vnode, container, parentAnchor, { o: { insert }, m: move }, moveType = 2) {
  ...
  if (!isReorder || isTeleportDisabled(props)) {
    if (shapeFlag & 16)
      for (let i = 0; i < children.length; i++) move(children[i], container, parentAnchor, 2)
  }
  ...
  if (key === 'class') {
    ...
    if (vnode.dirs) {
      for (const { dir, value } of vnode.dirs)
        if (dir.name === 'show' && !value) expectedMap.set('display', 'none')
    }
...
function isMismatchAllowed(el, allowedType) {
  if (allowedType === 0 || allowedType === 1)
    while (el && !el.hasAttribute(allowMismatchAttr)) el = el.parentElement
...
function injectHook(type, hook, target = currentInstance, prepend = false) {
  if (target) {
    ...
    warn$1(
      `${toHandlerKey(ErrorTypeStrings$1[type].replace(/ hook$/, ''))} is called when there is no active component instance to be associated with. Lifecycle injection APIs can only be used during execution of setup(). If you are using async setup(), make sure to register lifecycle hooks before the first await statement.`
    )
}
const createHook =
  lifecycle =>
...
function resolveAsset(type, name, warnMissing = true, maybeSelfReference = false) {
  ...
  if (instance) {
    ...
    const res =
      resolve(instance[type] || Component[type], name) || resolve(instance.appContext[type], name)
  ...
  get({ _: instance }, key) {
    ...
    if (publicGetter) {
      ...
      if (data !== EMPTY_OBJ && isReservedPrefix(key[0]) && hasOwn(data, key))
        warn$1(
          `Property ${JSON.stringify(key)} must be accessed via $data because it starts with a reserved character ("$" or "_") and is not proxied on the render context.`
        )
  ...
  Object.keys(toRaw(setupState)).forEach(key => {
    if (!setupState.__isScriptSetup) {
      if (isReservedPrefix(key[0])) {
        warn$1(
          `setup() return property ${JSON.stringify(key)} should not start with "$" or "_" which are reserved prefixes for Vue internals.`
        )
        return
      }
      Object.defineProperty(ctx, key, {
...
function applyOptions(instance) {
  ...
  if (watchOptions)
    for (const key in watchOptions) createWatcher(watchOptions[key], ctx, publicThis, key)
  if (provideOptions) {
...
function emit(instance, event, ...rawArgs) {
  ...
  devtoolsComponentEmit(instance, event, args)
  {
    ...
    if (lowerCaseEvent !== event && props[toHandlerKey(lowerCaseEvent)])
      warn$1(
        `Event "${lowerCaseEvent}" is emitted in component ${formatComponentName(instance, instance.type)} but the handler is registered for "${event}". Note that HTML attributes are case-insensitive and you cannot use v-on to listen to camelCase events when using in-DOM templates. You should probably use "${hyphenate(event)}" instead of "${event}".`
      )
  }
  let handlerName
  let handler =
...
const getFunctionalFallthrough = attrs => {
  ...
  for (const key in attrs)
    if (key === 'class' || key === 'style' || isOn(key)) (res || (res = {}))[key] = attrs[key]
  return res
}
const filterModelListeners = (attrs, props) => {
  const res = {}
  for (const key in attrs)
    if (!isModelListener(key) || !(key.slice(9) in props)) res[key] = attrs[key]
  return res
}
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const setScopeId = (el, vnode, scopeId, slotScopeIds, parentComponent) => {
    ...
    if (slotScopeIds)
      for (let i = 0; i < slotScopeIds.length; i++) hostSetScopeId(el, slotScopeIds[i])
    if (parentComponent) {
    ...
    if (isAsyncSetup) {
      ...
      if (isSSR)
        ...
        if (!instance.suspense)
          warn$1(
            `Component <${formatComponentName(instance, Component)}>: setup function returned a promise, but no <Suspense> boundary was found in the parent component tree. A component with async setup() must be nested in a <Suspense> in order to be rendered.`
          )
      }
...
function initCustomFormatter() {
  ...
  function extractKeys(instance, type) {
    ...
    for (const key in instance.ctx)
      if (isKeyOfType(Comp, key, type)) extracted[key] = instance.ctx[key]
    return extracted
  }
...
function resolveTransitionProps(rawProps) {
  ...
  for (const key in rawProps)
    if (!(key in DOMTransitionPropsValidators)) baseProps[key] = rawProps[key]
...
//#endregion
//#region node_modules/.pnpm/@vue+compiler-core@3.5.26/node_modules/@vue/compiler-core/dist/compiler-core.esm-bundler.js
/**
...
var Tokenizer = class {
  ...
  stateSpecialStartSequence(c) {
    ...
    if (!(isEnd ? isEndOfTagSection(c) : (c | 32) === this.currentSequence[this.sequenceIndex]))
      this.inRCDATA = false
    else if (!isEnd) {
      this.sequenceIndex++
      return
...
const tokenizer = new Tokenizer(stack, {
  ...
  onclosetag(start, end) {
    ...
    if (!currentOptions.isVoidTag(name)) {
      ...
      for (let i = 0; i < stack.length; i++)
        if (stack[i].tag.toLowerCase() === name.toLowerCase()) {
          found = true
  ...
  ondirname(start, end) {
    ...
    const name =
      raw === '.' || raw === ':' ? 'bind' : raw === '@' ? 'on' : raw === '#' ? 'slot' : raw.slice(2)
    ...
    if (inVPre || name === '')
      ...
      if (name === 'pre') {
        ...
        for (let i = 0; i < props.length; i++)
          if (props[i].type === 7) props[i] = dirToAttr(props[i])
      }
    }
  },
  ondirarg(start, end) {
      ...
      if (quote !== 0) {
        ...
        if (currentProp.type === 6) {
          ...
          if (
            ...
            (syncIndex = currentProp.modifiers.findIndex(mod => mod.content === 'sync')) > -1 &&
            checkCompatEnabled(
              'COMPILER_V_BIND_SYNC',
              currentOptions,
              currentProp.loc,
...
function onCloseTag(el, end, isImplied = false) {
  ...
  if (tokenizer.inXML && (stack[0] ? stack[0].ns : currentOptions.ns) === 0) tokenizer.inXML = false
  {
    ...
    if (
      inlineTemplateProp &&
      checkCompatEnabled('COMPILER_INLINE_TEMPLATE', currentOptions, inlineTemplateProp.loc) &&
      el.children.length
    )
...
function isComponent({ tag, props }) {
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    if (p.type === 6) {
      ...
      checkCompatEnabled('COMPILER_IS_ON_ELEMENT', currentOptions, p.loc)
    )
      return true
  }
  return false
...
function walk(node, parent, context, doNotHoistNode = false, inFor = false) {
  ...
  if (!cachedAsArray)
    for (const child of toCache) child.codegenNode = context.cache(child.codegenNode)
...
function generate(ast, options = {}) {
  ...
  push(
    `function ${ssr ? `ssrRender` : `render`}(${(ssr
...
`,
    ...
    if (ast.hoists.length)
      push(
        `const { ${[CREATE_VNODE, CREATE_ELEMENT_VNODE, CREATE_COMMENT, CREATE_TEXT, CREATE_STATIC]
...
function genExpressionAsPropertyKey(node, context) {
  ...
  if (node.type === 8) {
    ...
    push(isSimpleIdentifier(node.content) ? node.content : JSON.stringify(node.content), -2, node)
    ...
    if (prop.type === 6) {
      ...
      if (
        ...
        (isVBind &&
          ...
          (isComponentTag(tag) || isCompatEnabled('COMPILER_IS_ON_ELEMENT', context)))
      )
        continue
      ...
      case 15:
        ...
        if (!hasDynamicKey) {
          ...
          if (
            styleProp &&
            (hasStyleBinding ||
              (styleProp.value.type === 4 && styleProp.value.content.trim()[0] === `[`) ||
  ...
  for (let i = 0; i < node.props.length; i++) {
    ...
    if (p.type === 6) {
      ...
      if (p.exp) slotName = p.exp
        ...
        slotName = p.exp = createSimpleExpression(camelize(p.arg.content), false, p.arg.loc)
    } else {
  ...
  if (node.type === 1) {
    for (const prop of node.props)
      if (
        prop.type === 7 &&
        prop.name === 'bind' &&
        (!prop.exp || (prop.exp.type === 4 && !prop.exp.content.trim())) &&
        prop.arg
      ) {
        const arg = prop.arg
...
//#endregion
//#region node_modules/.pnpm/@vue+compiler-dom@3.5.26/node_modules/@vue/compiler-dom/dist/compiler-dom.esm-bundler.js
/**
...
const transformClick = (key, event) => {
  return isStaticExp(key) && key.content.toLowerCase() === 'onclick'
...
const transformTransition = (node, context) => {
  if (node.type === 1 && node.tagType === 1) {
    if (context.isBuiltInComponent(node.tag) === TRANSITION)
      return () => {
...
//#endregion
//#region node_modules/.pnpm/vue@3.5.26_typescript@5.9.3/node_modules/vue/dist/vue.esm-bundler.js
/**
 * vue v3.5.26
 * (c) 2018-present Yuxi (Evan) You and Vue contributors
 * @license MIT
