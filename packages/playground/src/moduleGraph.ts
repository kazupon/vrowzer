...
import type { ModuleInfo, PartialResolvedId } from '@rolldown/browser'
...
export type ResolvedUrl = [url: string, resolvedId: string, meta: object | null | undefined]
...
export class EnvironmentModuleGraph {
    ...
    mod.importers.forEach(importer => {
      if (!importer.acceptedHmrDeps.has(mod)) {
        ...
        const shouldSoftInvalidateImporter =
          (importer.staticImportedUrls?.has(mod.url) || softInvalidate) && importer.type === 'js'
  ...
  async updateModuleInfo(
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- FIX(kazupon): types
   ...
   * @internal
   */
  async _resolveUrl(url: string, alreadyResolved?: PartialResolvedId): Promise<ResolvedUrl> {
