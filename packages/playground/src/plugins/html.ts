...
import { getNodeAssetAttributes } from '../assetSource.ts'
import { toOutputFilePathInHtml } from '../build.ts'
import { resolveEnvPrefix } from '../env.ts'
import { checkPublicFile } from '../publicDir.ts'
import { cleanUrl } from '../shared/utils.ts'
import { perEnvironmentState } from '../ssr/environment.ts'
import {
  decodeURIIfPossible,
  encodeURIPath,
  generateCodeFrame,
...
import { assetUrlRE, getPublicAssetFilename, publicAssetUrlRE, urlToBuiltUrl } from './asset.ts'
import { cssBundleNameCache } from './css.ts'
import { modulePreloadPolyfillId } from './modulePreloadPolyfill.ts'

import type { OutputAsset, OutputChunk, RollupError, SourceMapInput } from '@rolldown/browser'
import type { DefaultTreeAdapterMap, ErrorCodes, ParserError, Token } from 'parse5'
import type {
  HtmlTagDescriptor,
...
import type { MinimalPluginContextWithoutEnvironment } from '../plugin.ts'
...
export function removeViteIgnoreAttr(
  ...
  const loc = (sourceCodeLocation as Token.LocationWithAttributes).attrs?.['vite-ignore']
  if (loc) {
...
export function buildHtmlPlugin(config: ResolvedConfig): Plugin {
  ...
  const isExcludedUrl = (url: string) => url[0] === '#' || isExternalUrl(url) || isDataUrl(url)
  ...
  return {
    ...
    transform: {
      ...
      async handler(html, id) {
        ...
        const nodeStartWithLeadingWhitespace = (node: DefaultTreeAdapterMap['node']) => {
        ...
        await traverseHtml(html, id, config.logger.warn, node => {
                ...
                if (url && !isExcludedUrl(url) && !isPublicFile) {
                  setModuleSideEffectPromises.push(
                    this.resolve(url, id).then(resolved => {
                      if (!resolved) {
                        return Promise.reject(new Error(`Failed to resolve ${url} from ${id}`))
                      }
                      // set moduleSideEffects to keep the module even if `treeshake.moduleSideEffects=false` is set
                  ...
                  const scriptNode = node.childNodes.pop() as DefaultTreeAdapterMap['textNode']
              ...
              if (url === undefined) {
                ...
                if (
                  ...
                  const isNoInlineLink =
                    node.nodeName === 'link' &&
                    attr.attributes.rel &&
                    parseRelAttr(attr.attributes.rel).some(v => noInlineLinkRels.has(v))
                  const shouldInline = isNoInlineLink ? false : undefined
                  assetUrlsPromises.push(
                    (async () => {
                      const processedUrl = await processAssetUrl(url, shouldInline)
                      if (processedUrl !== url) {
                        overwriteAttrValue(s, attr.location, partialEncodeURIPath(processedUrl))
                      }
                    })()
                  )
                }
          ...
          if (inlineStyle) {
            ...
            // TODO(kazupon): use altnertative getHash with web standard crypto
            // const hash = getHash(cleanUrl(id))
            const hash = cleanUrl(id)
          ...
          if (node.nodeName === 'style' && node.childNodes.length) {
            const styleNode = node.childNodes.pop() as DefaultTreeAdapterMap['textNode']
            const filePath = id.replace(normalizePath(config.root), '')
            inlineModuleIndex++
            addToHTMLProxyCache(config, filePath, inlineModuleIndex, {
    ...
    async generateBundle(options, bundle) {
      ...
      const getCssFilesForChunk = (
        ...
        // @ts-expect-error -- FIXME(kazupon): viteMetadata is missing in types
      ...
      const getCssTagsForChunk = (chunk: OutputChunk, toOutputPath: (filename: string) => string) =>
        getCssFilesForChunk(chunk).map(file => toStyleSheetLinkTag(file, toOutputPath))
      ...
      for (const [normalizedId, html] of processedHtml(this)) {
        ...
        const toOutputFilePath = (filename: string, type: 'asset' | 'public') => {
        ...
        const toOutputAssetFilePath = (filename: string) => toOutputFilePath(filename, 'asset')
          ...
          if (canInlineEntry) {
            ...
            if (modulePreload !== false) {
              const resolveDependencies =
                typeof modulePreload === 'object' && modulePreload.resolveDependencies
        ...
        if (!this.environment.config.build.cssCodeSplit) {
          ...
          const cssChunk =
            ...
            (Object.values(bundle).find(
              chunk => chunk.type === 'asset' && chunk.names.includes(cssBundleName)
        ...
        result = result.replace(publicAssetUrlRE, (_, fileHash) => {
          ...
          return encodeURIPath(
            URL.canParse(publicAssetPath) ? publicAssetPath : normalizePath(publicAssetPath)
          )
        })
...
export function preImportMapHook(config: ResolvedConfig): IndexHtmlTransformHook {
  return (html, ctx) => {
...
export function injectCspNonceMetaTagHook(config: ResolvedConfig): IndexHtmlTransformHook {
  return () => {
...
export function injectNonceAttributeTagHook(config: ResolvedConfig): IndexHtmlTransformHook {
...
function headTagInsertCheck(tags: HtmlTagDescriptor[], ctx: IndexHtmlTransformContext) {
