...
import { blankReplacer, createDebugger } from './utils.ts'

import type { ExistingRawSourceMap, SourceMap } from '@rolldown/browser'
...
export function getCodeWithSourcemap(type: 'js' | 'css', code: string, map: SourceMap): string {
  if (debug) {
...
export async function extractSourcemapFromFile(
  ...
  const map = (
    ...
    (await convertSourceMap.fromMapFileSource(code, createConvertSourceMapReadMap(filePath)))
  )?.toObject()
