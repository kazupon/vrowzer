import { monotonicDateNow } from './utils.ts'

import type { ModuleInfo } from '@rolldown/browser'
import type { TransformResult } from 'vite'
import type { EnvironmentModuleGraph, EnvironmentModuleNode, ResolvedUrl } from './moduleGraph.ts'
...
export class ModuleGraph {
  ...
  constructor(moduleGraphs: {
    ...
    const getModuleMapUnion = (prop: 'urlToModuleMap' | 'idToModuleMap') => () => {
      // A good approximation to the previous logic that returned the union of
      // the importedModules and importers from both the browser and server
  ...
  async getModuleByUrl(url: string, _ssr?: boolean): Promise<ModuleNode | undefined> {
  ...
  invalidateModule(
    ...
    if (mod._ssrModule) {
      ...
      this._ssr.invalidateModule(
        mod._ssrModule,
        new Set([...seen].map(mod => mod._ssrModule).filter(Boolean)) as Set<EnvironmentModuleNode>,
        timestamp,
        isHmr,
        softInvalidate
      )
...
function createBackwardCompatibleFileToModulesMap(
  ...
  const getBackwardCompatibleModules = (modules: Set<EnvironmentModuleNode>): Set<ModuleNode> =>
    new Set([...modules].map(mod => moduleGraph.getBackwardCompatibleModuleNode(mod)))
