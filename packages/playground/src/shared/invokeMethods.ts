...
export type InvokeSendData<T extends keyof InvokeMethods = keyof InvokeMethods> = {
  name: T
...
export type InvokeResponseData<T extends keyof InvokeMethods = keyof InvokeMethods> = {
  name: T
...
export type InvokeMethods = {
  ...
  getBuiltins: () => Promise<
    Array<{ type: 'string'; value: string } | { type: 'RegExp'; source: string; flags: string }>
  >
}
