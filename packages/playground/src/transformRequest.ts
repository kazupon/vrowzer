...
import { isFileLoadingAllowed } from './middlewares/static.ts'
import { throwClosedServerError } from './pluginContainer.ts'
import { checkPublicFile } from './publicDir.ts'
import { cleanUrl, slash, unwrapId } from './shared/utils.ts'
import {
  applySourcemapIgnoreList,
  extractSourcemapFromFile,
  injectSourcesContent
} from './sourcemap.ts'
import { ssrTransform } from './ssr/ssrTransform.ts'
import {
  createDebugger,
  ensureWatchedFile,
  injectQuery,
...
import type { ModuleType, PartialResolvedId, SourceDescription, SourceMap } from '@rolldown/browser'
...
async function loadAndTransform(
  ...
  if (loadResult == null) {
    ...
    if (
      ...
      if (code != null && environment.pluginContainer.watcher) {
        ensureWatchedFile(environment.pluginContainer.watcher, file, config.root)
      }
    }
    if (code) {
      try {
  ...
  if (code == null) {
    ...
    const msg = isPublicFile
      ? `This file is in ${publicDirName} and will be copied as-is during ` +
        ...
        `via HTML tags.`
      : `Does the file exist?`
    const importerMod: EnvironmentModuleNode | undefined = moduleGraph.idToModuleMap
      .get(id)
      ?.importers.values()
      .next().value
  ...
  const result = environment.config.dev.moduleRunnerTransform
    ? await ssrTransform(code, normalizedMap, url, originalCode, {
        json: {
          stringify:
            topLevelConfig.json.stringify === true && topLevelConfig.json.namedExports !== true
        }
      })
    : ({
        code,
...
async function handleModuleSoftInvalidation(
  ...
  // @ts-expect-error -- FIXME(kazupon): internal field
  ...
  // We need to transform each imports with new timestamps if available
    ...
    for (const imp of imports) {
      ...
      const hmrUrl = unwrapId(
        stripBase(removeImportQuery(urlWithoutTimestamp), environment.config.base)
      )
