 ...
 * Vrowser E2E Tests
 *
 * Verifies that the vrowser library works end-to-end in a browser environment.
 * Builds a test playground with Vite + Vrowser, starts vite preview server,
...
describe('Vrowser E2E', () => {
    ...
    test('preview content is rendered in iframe', async () => {
      ...
      await page.waitForFunction(
        () => {
          ...
          return iframe?.contentDocument?.body?.innerText?.includes('Hello from Vrowser!')
        },
        { timeout: 30000 }
      )
      ...
      expect(text).toContain('Hello from Vrowser!')
  ...
  describe('file operations', () => {
    test('updateFile() triggers preview update', async () => {
      // Update main.js via vrowser API
      await page.evaluate(() => {
        const vrowser = (window as any).__vrowser__
        vrowser.updateFile(
          '/main.js',
          `
