...
function getImportBindings(node: Directive | Statement): ImportBindings | undefined {
...
function replaceConfusingTypeNames(
  ...
  if (!isInternalEntry) {
    for (const modName in identifierReplacements) {
      ...
      if (imp.length === 0) {
        this.warn(`${chunk.fileName} does not import "${modName}" for replacement`)
        // TODO(kazupon): consider failing the build
        // process.exitCode = 1
        continue
      }
      ...
      for (const id in replacements) {
        ...
        if (!imp.some(i => i.locals.includes(id))) {
          this.warn(`${chunk.fileName} does not import "${id}" from "${modName}" for replacement`)
          // TODO(kazupon): consider failing the build
          // process.exitCode = 1
          continue
        }
        ...
        if (betterId.includes('.')) {
          chunk.code = chunk.code.replace(new RegExp(`\\b\\w+\\b as ${regexEscapedId},?\\s?`), '')
        }
        chunk.code = chunk.code.replace(new RegExp(`\\b${regexEscapedId}\\b`, 'g'), betterId)
      }
    }
  }

  const identifiers = unique(
    Array.from(chunk.code.matchAll(identifierWithTrailingDollarRE), m => m[0])
  )
  const unreplacedIds = identifiers.filter(id => !ignoreConfusingTypeNames.includes(id))
  if (unreplacedIds.length) {
    const unreplacedStr = unreplacedIds.map(id => `\n- ${id}`).join('')
    this.warn(`${chunk.fileName} contains confusing identifier names${unreplacedStr}`)
    // TODO(kazupon): consider failing the build
    // process.exitCode = 1
  }
  ...
  if (!isInternalEntry) {
    ...
    if (notUsedConfusingTypeNames.length) {
      const notUsedStr = notUsedConfusingTypeNames.map(id => `\n- ${id}`).join('')
