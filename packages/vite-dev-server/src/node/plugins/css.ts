...
// NOTE(kazupon): disalbe now, because artichokie doesn't work in browser currently, we need to implement our own worker pool later.
...
export { isDirectCSSRequest } from '../utils'
...
// NOTE(kazupon): comment out, because we want to keep the original code to maintain with LLM's
...
// oxlint-disable-next-line typescript/no-unused-vars -- bug in the TypeScript lint rule
...
// oxlint-disable-next-line regexp/no-unused-capturing-group -- doesn't detect asyncReplace usage
...
async function minifyCSS(
  ...
  /**
   * NOTE(kazupon): disable css minify, because both esbuild and lightningcss will be supprted in browser
...
const scssProcessor = (
  ...
  return {
    ...
    async process(environment, source, root, options, resolvers) {
      return { code: source, deps: [] }
    }
  }
}
// NOTE(kazupon): disable scss preprocessor for now, because we cannot support currently in brwoser
// const scssProcessor = (
//   maxWorkers: number | undefined,
// ): StylePreprocessor<SassStylePreprocessorInternalOptions> => {
...
// NOTE(kazupon): disable less preprocessor for now, because we cannot support currently in brwoser
// const lessProcessor = (
//   maxWorkers: number | undefined,
// ): StylePreprocessor<LessStylePreprocessorInternalOptions> => {
...
// NOTE(kazupon): disable stylus preprocessor for now, because we cannot support currently in brwoser
// const stylProcessor = (
//   maxWorkers: number | undefined,
// ): StylePreprocessor<StylusStylePreprocessorInternalOptions> => {
...
const importEsbuild = createCachedImport(() => Promise.resolve({ transform: () => { }, formatMessages: () => { } } as unknown as typeof import('esbuild')))
const importLightningCSS = createCachedImport(() => Promise.resolve({ bundleAsync: () => { } } as unknown as typeof import('lightningcss')))
// NOTE(kazupon): disable now, because we don't need esbuild and lightningcss in browser now, these are stubbed for now.
...
async function compileLightningCSS(
  ...
  // NOTE(kazupon): disable lightningcss for now, because we don't need it in browser currently
