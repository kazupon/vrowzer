...
import { cleanUrl, generateEtag, slash, unwrapId } from '../../shared/utils'
...
async function doTransform(
  ...
  // TODO(kazupon): disalbe optimizer, because we don't still implement it.
...
async function loadAndTransform(
  ...
  const result = environment.config.dev.moduleRunnerTransform
    ...
    : ({
      code,
      map: normalizedMap,
      etag: generateEtag(code, { weak: true }),
      // NOTE(kazupon): keep the original codes, because we need to maintain forked codes from original codes later with LLMs.
