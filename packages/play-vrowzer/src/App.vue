...
const manifestLoaders = import.meta.glob<{ default: VrowzerManifest }>(
  '../fixtures/*/vrowzer-manifest.json',
  { query: '?vrowzer' }
)
...
const activeManifest = ref<VrowzerManifest | null>(null)
...
vrowzer.on('reloadSuggested', info => {
  window.alert(
...
onMounted(async () => {
  ...
  if (!editorPanel.value) {
    console.warn('[Vrowzer Playground] EditorPanel ref not available, using manifest directly')
...
<template>
  <div class="app">
    <header class="app-header">
      <img src="/favicon.svg" alt="Vrowzer" class="app-logo" />
      <h1>Vrowzer Playground</h1>
