...
| `waitForController` _(optional)_ | `boolean` | Wait for the service worker to become the page controller. When `true`, `ready()` will send a `V_SW_CLAIM_CLIENTS` message to the active SW and wait for `navigator.serviceWorker.controller` to become non-null before returning `true`. When `false` (default), `ready()` returns `true` as soon as the SW is active, even if it's not yet the controller. A `reloadSuggested` event is emitted in this case. **Default:** `false` |
